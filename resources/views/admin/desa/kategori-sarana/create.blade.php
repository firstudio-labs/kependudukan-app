<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">Tambah Kategori Sarana</h1>
        <p class="text-sm text-gray-500 mb-6">Pilih jenis sarana dan masukkan kategori.</p>

        <form method="POST" action="{{ route('admin.desa.kategori-sarana.store') }}" class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <select name="jenis_sarana" required class="mt-1 block w-full rounded-lg border border-gray-300 p-2.5 focus:border-[#7886C7] focus:ring-2 focus:ring-[#7886C7]">
                        ...
                        @foreach($allowedJenis as $j)
                            <option value="{{ $j }}" {{ old('jenis_sarana')===$j ? 'selected' : '' }}>{{ $j }}</option>
                        @endforeach
                    </select>
                ...
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <input type="text" name="kategori" value="{{ old('kategori') }}" required placeholder="Misal: Sekolah, Masjid, Posyandu, ..."
