<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Usaha Desa</h1>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <form method="GET" action="" class="flex items-center gap-3">
                ...
                <div class="relative flex items-center gap-2">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama/ijin/ketua/tahun..."
            ...
            <a href="{{ route('admin.desa.usaha.create') }}" class="text-white bg-[#7886C7] hover:bg-[#2D336B] focus:ring-4 focus:ring-[#5C69A7] font-medium rounded-lg text-sm px-5 py-2.5">Tambah Usaha</a>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-[#e6e8ed]">
                    <tr>
                        ...
                        <th class="px-6 py-3">Ijin</th>
                        <th class="px-6 py-3">Tahun Didirikan</th>
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">{{ $item->ijin ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $item->tahun_didirikan ?? '-' }}</td>
                            <td class="px-6 py-4">{{ $item->ketua ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.desa.usaha.edit', $item) }}" class="text-yellow-600 hover:text-yellow-800 mr-3"><i class="fa-solid fa-pen-to-square"></i></a>
                                <form action="{{ route('admin.desa.usaha.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?')">
                                    @csrf
                                    @method('DELETE')
