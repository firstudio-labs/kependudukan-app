<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Tambah Pengumuman</h1>

        <form method="POST" action="{{ route('admin.desa.pengumuman.store') }}" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        ...
                        <input type="file" id="gambar" name="gambar" class="hidden" accept="image/png,image/jpeg,image/jpg" onchange="previewImagePengumuman(this)" aria-describedby="gambar_help_pengumuman gambar_filename_pengumuman">
                        <span id="gambar_filename_pengumuman" class="ml-3 text-sm text-gray-600 align-middle">Belum ada file dipilih</span>
                    </div>
                    <p id="gambar_help_pengumuman" class="mt-2 text-sm text-gray-500">PNG, JPG (MAX. 4MB).</p>
                    <div id="preview" class="mt-2">
                        <img id="preview_image_pengumuman" src="#" alt="Preview" class="hidden max-w-xs rounded-lg shadow-md">
                    </div>
                    @error('gambar')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="8"
                        class="mt-1 block w-full rounded-lg border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-3">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
    ...
    <script>
        function previewImagePengumuman(input) {
            const img = document.getElementById('preview_image_pengumuman');
            const name = document.getElementById('gambar_filename_pengumuman');
