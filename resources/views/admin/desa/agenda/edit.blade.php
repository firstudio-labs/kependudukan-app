<x-layout>
    <div class="p-4 mt-14">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Agenda Desa</h1>
            <a href="{{ route('admin.desa.agenda.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Kembali</a>
        </div>

        <form action="{{ route('admin.desa.agenda.update', $agenda->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-gray-600 text-sm font-medium mb-1">Judul</label>
                    <input type="text" name="judul" class="w-full p-2 border border-gray-300 rounded" value="{{ old('judul', $agenda->judul) }}" required>
                ...
                <div>
                    ...
                    <img id="preview-agenda-gambar" src="{{ $agenda->gambar ? asset('storage/'.$agenda->gambar) : '' }}" class="h-24 mt-2 {{ $agenda->gambar ? '' : 'hidden' }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="deskripsi" name="deskripsi" class="w-full p-2 border border-gray-300 rounded" rows="8">{{ old('deskripsi', $agenda->deskripsi) }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" id="tag_lokasi" name="tag_lokasi" class="w-full p-2 border border-gray-300 rounded" value="{{ old('tag_lokasi', $agenda->tag_lokasi) }}" placeholder="-6.2,106.8">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="alamat" name="alamat" rows="3" class="w-full p-2 border border-gray-300 rounded">{{ old('alamat', $agenda->alamat) }}</textarea>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (tagLokasiInput && tagLokasiInput.value) {
                ...
                if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) { defaultLat = parts[0]; defaultLng = parts[1]; }
            }
