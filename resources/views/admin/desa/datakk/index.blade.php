<x-layout>
    <div class="p-4 mt-14">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex space-x-2">
                <!-- Tombol Export -->
                <form method="GET" action="{{ route('admin.desa.datakk.index') }}">
                    @csrf
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                        ...
                        <td class="flex items-center px-6 py-4 space-x-2">
                            ...
                            <a href="{{ route('admin.desa.datakk.edit', $k['kk']) }}" class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            {{-- <a href="{{ route('admin.desa.datakk.update', $k['id'] ?? $k['kk']) }}" class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <form action="{{ route('admin.desa.destroy', $k['id'] ?? $k['kk']) }}" method="POST" class="delete-form" id="delete-form-{{ $k['id'] ?? $k['kk'] }}">
                                @csrf
                                @method('DELETE')
            ...
            @if(isset($kk['data']['pagination']))
                @php
                    ...
                    $paginationData = new \Illuminate\Pagination\LengthAwarePaginator(
                        $kk['data']['citizens'] ?? [],
                        $kk['data']['pagination']['total_items'] ?? 0,
                        $kk['data']['pagination']['items_per_page'] ?? 10,
                        $kk['data']['pagination']['current_page'] ?? 1,
                        [
                            'path' => request()->url(),
                            'query' => request()->query(),
                        ]
                    ...
                    // Ensure paginator knows there's at least 1 page
            ...
            @else
                <div class="px-4 py-3 text-sm text-gray-700">
                    Showing {{ count($kk['data']['citizens'] ?? []) }} results
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Load foto untuk setiap anggota keluarga yang sudah ada di tabel
            // Ini akan dijalankan setelah fetchFamilyMembers selesai
        });
        ...
        // Modifikasi fetchFamilyMembers untuk menambahkan foto
        function fetchFamilyMembers(kkNumber) {
            // Check if kkNumber is valid
            if (!kkNumber) {
                document.getElementById('familyMembersLoading').classList.add('hidden');
