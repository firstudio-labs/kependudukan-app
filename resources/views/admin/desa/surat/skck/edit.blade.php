<x-layout>
    <x-surat.form-layout-edit
        title="Edit Surat SKCK"
        route="{{ route('admin.desa.surat.skck.update', $skck->id) }}"
        :jobs="$jobs"
        :provinces="$provinces"
        :data="$skck"
        :signers="$signers">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    if (signingDropdown) {
                        ...
                        signingDropdown.innerHTML += `<option value="{{ $signer->id }}" {{ $skck->signing == $signer->id ? 'selected' : '' }}>{{ $signer->judul }} - {{ $signer->keterangan }}</option>`;
                        @endforeach
                    }
