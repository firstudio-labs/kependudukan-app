<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Surat SKCK</h1>
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <button
                type="button"
                onclick="window.location.href='{{ route('admin.desa.surat.skck.create') }}'"
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-[#e6e8ed]">
                    <tr>
                        ...
                        <th class="px-6 py-3">Dipergunakan Untuk</th>
                ...
                <tbody>
                    @forelse($skckList as $index => $skck)
                    <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">{{ $skck->nik }}</td>
                        <td class="px-6 py-4">{{ $skck->full_name }}</td>
                        <td class="px-6 py-4">
                            <div class="max-w-xs truncate" title="{{ $skck->purpose }}">
                                {{ \Illuminate\Support\Str::limit($skck->purpose, 50, '...') }}
                            </div>
                        </td>
                        ...
                        <td class="flex items-center px-6 py-4 space-x-2">
                            ...
                            @endif
                            <a href="{{ route('admin.desa.surat.skck.pdf', $skck->id) }}" class="text-blue-600 hover:text-blue-800" target="_blank" aria-label="Export PDF">
                                <i class="fa-solid fa-file-pdf"></i>
                            </a>
                            @if(!$skck->is_accepted)
                                <a href="{{ route('admin.desa.surat.skck.edit', $skck->id) }}" class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                            @endif
                            <form action="{{ route('admin.desa.surat.skck.delete', $skck->id) }}" method="POST" onsubmit="return confirmDelete(event)">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="text-center py-4">Tidak ada data surat SKCK.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="px-4 py-3 flex flex-col sm:flex-row justify-between items-center">
                <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                    @php
                        $pagination = [
                            'current_page' => $skckList->currentPage(),
                            'items_per_page' => $skckList->perPage(),
                            'total_items' => $skckList->total()
                        ];
                ...
                @if($skckList->lastPage() > 1)
                    <nav class="relative z-0 inline-flex shadow-sm -space-x-px" aria-label="Pagination">
                        @php
                            $totalPages = $skckList->lastPage();
                            $currentPage = $skckList->currentPage();
