<!-- filepath: c:\laragon\www\kependudukan-app\resources\views\admin.desa\datamaster\surat\penandatangan\index.blade.php -->
<x-layout>
    <div class="p-4 mt-14">
        ...
        <div class="flex justify-between items-center mb-4">
            <!-- Input Pencarian -->
            <form method="GET" action="{{ route('admin.desa.datamaster.surat.penandatangan.index') }}" class="relative">
                <input
                    type="text"
                    name="search"
                    id="search"
            ...
            <button
                type="button"
                onclick="window.location.href='{{ route('admin.desa.datamaster.surat.penandatangan.create') }}'"
                    ...
                    @forelse($penandatangans as $index => $penandatangan)
                        <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                            ...
                            <td class="flex items-center px-6 py-4 space-x-2">
                                <a href="{{ route('admin.desa.datamaster.surat.penandatangan.edit', $penandatangan->id) }}" class="text-yellow-600 hover:text-yellow-800">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>

                                <form id="delete-form-{{ $penandatangan->id }}" action="{{ route('admin.desa.datamaster.surat.penandatangan.destroy', $penandatangan->id) }}" method="POST" onsubmit="confirmDelete(event, {{ $penandatangan->id }})">
                                    @csrf
                                    @method('DELETE')
