<x-layout>
    <div class="p-4 mt-14">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <button
                type="button"
                onclick="window.location.href='{{ route('admin.desa.surat.ahli-waris.create') }}'"
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                        ...
                        <td class="flex items-center px-6 py-4 space-x-2">
                            ...
                            @endif
                            <a href="{{ route('admin.desa.surat.ahli-waris.pdf', $ahliWaris->id) }}" class="text-blue-600 hover:text-blue-800" target="_blank" aria-label="Export PDF">
                                <i class="fa-solid fa-file-pdf"></i>
                            </a>
                            @if(!$ahliWaris->is_accepted)
                                <a href="{{ route('admin.desa.surat.ahli-waris.edit', $ahliWaris->id) }}" class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                            @endif
                            <form action="{{ route('admin.desa.surat.ahli-waris.delete', $ahliWaris->id) }}" method="POST" onsubmit="return confirmDelete(event)">
                                @csrf
                                @method('DELETE')
            ...
            <div class="px-4 py-3 flex flex-col sm:flex-row justify-between items-center">
                <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                    @php
                        $pagination = [
                            'current_page' => $ahliWarisList->currentPage(),
                            'items_per_page' => $ahliWarisList->perPage(),
                            'total_items' => $ahliWarisList->total()
                        ];
                ...
                @if($ahliWarisList->lastPage() > 1)
                    <nav class="relative z-0 inline-flex shadow-sm -space-x-px" aria-label="Pagination">
                        @php
                            $totalPages = $ahliWarisList->lastPage();
                            $currentPage = $ahliWarisList->currentPage();
