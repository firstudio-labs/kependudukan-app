<x-layout>
    <div class="p-4 mt-14">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <button
                type="button"
                onclick="window.location.href='{{ route('admin.desa.surat.kematian.create') }}'"
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                        ...
                        <td class="flex items-center px-6 py-4 space-x-2">
                            @if($kematian->is_accepted)
                                <span class="text-green-600 font-semibold">Acceptance</span>
                            @endif
                            <a href="{{ route('admin.desa.surat.kematian.pdf', $kematian->id) }}" class="text-blue-600 hover:text-blue-800" aria-label="Export PDF" target="_blank">
                                <i class="fa-solid fa-file-pdf"></i>
                            </a>
                            @if(!$kematian->is_accepted)
                                <a href="{{ route('admin.desa.surat.kematian.edit', $kematian->id) }}" class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                            @endif
                            <form action="{{ route('admin.desa.surat.kematian.delete', $kematian->id) }}" method="POST" onsubmit="return confirmDelete(event)">
                                @csrf
                                @method('DELETE')
