<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Surat Izin Rumah Sewa</h1>
        ...
        <div class="flex justify-between items-center mb-4">
            <form method="GET" action="" class="relative w-full max-w-xs">
                <input
                    ...
                    placeholder="Cari izin rumah sewa..."
                />
            ...
            <button
                type="button"
                onclick="window.location.href='{{ route('admin.desa.surat.rumah-sewa.create') }}'"
            ...
            >
                ...
                <span>Tambah Izin Rumah Sewa</span>
            </button>
        </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-[#e6e8ed]">
                    <tr>
                        ...
                        <th class="px-6 py-3">Nama Penyelenggara</th>
                        <th class="px-6 py-3">Nama Penanggung Jawab</th>
                        <th class="px-6 py-3">Alamat Rumah Sewa</th>
                        <th class="px-6 py-3">Pejabat Penandatangan</th>
                        <th class="px-6 py-3">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($rumahSewaList as $index => $rumahSewa)
                    <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">{{ $rumahSewa->nik ?? '-' }}</td>
                        ...
                        <td class="flex items-center px-6 py-4 space-x-2">
                            ...
                            @endif
                            <a href="{{ route('admin.desa.surat.rumah-sewa.pdf', $rumahSewa->id) }}" class="text-blue-600 hover:text-blue-800" aria-label="Export PDF" target="_blank">
                                <i class="fa-solid fa-file-pdf"></i>
                            </a>
                            @if(!$rumahSewa->is_accepted)
                                <a href="{{ route('admin.desa.surat.rumah-sewa.edit', $rumahSewa->id) }}" class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                            @endif
                            <form action="{{ route('admin.desa.surat.rumah-sewa.delete', $rumahSewa->id) }}" method="POST" onsubmit="return confirmDelete(event)">
                                @csrf
                                @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="text-center py-4">Tidak ada data izin rumah sewa.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <!-- Pagination Section -->
            @if($rumahSewaList->hasPages())
            <div class="px-4 py-3 flex flex-col sm:flex-row justify-between items-center">
                <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                    @php
                        $pagination = [
                            'current_page' => $rumahSewaList->currentPage(),
                            'items_per_page' => $rumahSewaList->perPage(),
                            'total_items' => $rumahSewaList->total()
                        ];
                ...
                @if($rumahSewaList->lastPage() > 1)
                    <nav class="relative z-0 inline-flex shadow-sm -space-x-px" aria-label="Pagination">
                        @php
                            $totalPages = $rumahSewaList->lastPage();
                            $currentPage = $rumahSewaList->currentPage();
