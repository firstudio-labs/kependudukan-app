...
async function checkKKExists(kk) {
    try {
        ...
        kkInput.disabled = true; // Disable sementara saat loading
        ...
        if (data.exists) {
            ...
            kkInput.style.borderColor = '#10B981'; // Green border

        } else {
            // KK doesn't exist, enable form for manual entry
            Swal.fire({
                icon: 'success',
                title: 'Nomor KK Baru',
            ...
            kkInput.style.borderColor = '#3B82F6'; // Blue border
        }

    } catch (error) {
        console.error('Error checking KK:', error);
        kkInput.style.borderColor = '#EF4444'; // Red border
        kkInput.disabled = false;
        kkInput.style.backgroundColor = '#FFFFFF';
...
function fillFormWithKKData(kkData, familyMembers) {
    ...
    // Set hidden inputs with IDs first
    ...
    // Get province code by matching province_id with data-id attribute
    if (kkData.province_id) {
        const provinceSelect = document.getElementById('province_code');
        let provinceCode = null;

        // Loop through all province options to find matching ID
...
function loadAndSelectByDataId(selectId, targetId, callback) {
    ...
    // Determine endpoint based on select ID
    ...
    $.get(`${url}${endpoint}`)
        .done(function(data) {
            if (data && data.length > 0) {
                ...
                // Find and select the matching item by data-id
                ...
                // Call callback after everything is done
                if (callback) {
                    callback();
                }
            }
...
// Helper function to load and select district
function loadDistrictsForAutoFill(provinceCode, selectedDistrictCode, callback) {
    if (!selectedDistrictCode) {
        if (callback) callback();
        return;
    }

    $.get(`${getBaseUrl()}/location/districts/${provinceCode}`, function(districts) {
        if (districts && districts.length > 0) {
            // Populate district select
            let districtHtml = '<option value="">Pilih Kabupaten</option>';
            districts.forEach(district => {
                districtHtml += `<option value="${district.code}" data-id="${district.id}">${district.name}</option>`;
            });
            $('#district_code').html(districtHtml);

            // Select the district
            $('#district_code').val(selectedDistrictCode);

            // Set district_id from selected option
            const selectedDistrict = districts.find(d => d.code === selectedDistrictCode || d.id == selectedDistrictCode);
            if (selectedDistrict) {
                $('#district_id').val(selectedDistrict.id);
            }
...
// Helper function to load and select sub-district
function loadSubDistrictsForAutoFill(districtCode, selectedSubDistrictCode, callback) {
    if (!selectedSubDistrictCode || !districtCode) {
        if (callback) callback();
        return;
    }

    $.get(`${getBaseUrl()}/location/sub-districts/${districtCode}`, function(subDistricts) {
        if (subDistricts && subDistricts.length > 0) {
            // Populate sub-district select
            let subDistrictHtml = '<option value="">Pilih Kecamatan</option>';
            subDistricts.forEach(subDistrict => {
                subDistrictHtml += `<option value="${subDistrict.code}" data-id="${subDistrict.id}">${subDistrict.name}</option>`;
            });
            $('#sub_district_code').html(subDistrictHtml);

            // Select the sub-district
            $('#sub_district_code').val(selectedSubDistrictCode);

            // Set sub_district_id from selected option
            const selectedSubDistrict = subDistricts.find(sd => sd.code === selectedSubDistrictCode || sd.id == selectedSubDistrictCode);
            if (selectedSubDistrict) {
                $('#sub_district_id').val(selectedSubDistrict.id);
            }

            $('#sub_district_code').prop('disabled', false);
            $('#sub_district_code').trigger('change');
...
// Helper function to load and select village
function loadVillagesForAutoFill(subDistrictCode, selectedVillageCode) {
    if (!selectedVillageCode || !subDistrictCode) {
        return;
    }

    $.get(`${getBaseUrl()}/location/villages/${subDistrictCode}`, function(villages) {
        if (villages && villages.length > 0) {
            // Populate village select
            let villageHtml = '<option value="">Pilih Desa/Kelurahan</option>';
            villages.forEach(village => {
                villageHtml += `<option value="${village.code}" data-id="${village.id}">${village.name}</option>`;
            });
            $('#village_code').html(villageHtml);

            // Select the village
            $('#village_code').val(selectedVillageCode);

            // Set village_id from selected option
            const selectedVillage = villages.find(v => v.code === selectedVillageCode || v.id == selectedVillageCode);
            if (selectedVillage) {
                $('#village_id').val(selectedVillage.id);
            }

            $('#village_code').prop('disabled', false);
            $('#village_code').trigger('change');
        }
    });
}
...
function displayExistingFamilyMembers(members) {
    ...
    members.forEach((member, index) => {
        ...
        // Pastikan menggunakan family_status bukan status
...
function getFamilyStatusText(status) {
    // Handle jika status adalah string langsung
...
function clearKKForm() {
    ...
    // Clear family members container
    const container = document.getElementById('familyMembersContainer');
...
// Function to enable form for new entry
function enableForm() {
    kkInput.disabled = false;
    kkInput.style.backgroundColor = '#FFFFFF';

    // Enable all form fields
    const formFields = document.querySelectorAll('#familyMemberFormFields input, #familyMemberFormFields select');
    formFields.forEach(field => {
        field.disabled = false;
    });
}

// Function to disable form when KK exists
function disableFormForExistingKK() {
    // Disable family member form temporarily
    const formFields = document.querySelectorAll('#familyMemberFormFields input, #familyMemberFormFields select, #familyMemberFormFields textarea');
    formFields.forEach(field => {
        field.disabled = true;
...
// Add button to enable editing
function addEditButton() {
    const container = document.querySelector('.p-3.sm\\:p-4.mt-12.sm\\:mt-14');
    if (!document.getElementById('enableEditBtn')) {
        const editBtn = document.createElement('button');
        editBtn.id = 'enableEditBtn';
        editBtn.className = 'inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-yellow-600 text-base font-medium text-white hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-yellow-500 sm:text-lg mb-4';
        editBtn.textContent = 'Edit Data KK';
        editBtn.onclick = function() {
            kkInput.disabled = false;
            kkInput.style.backgroundColor = '#FFFFFF';

            const formFields = document.querySelectorAll('input:not(#kk), select, textarea');
            formFields.forEach(field => {
                field.disabled = false;
        ...
        const kkSection = document.querySelector('.bg-white.p-4.sm\\:p-6.rounded-lg.shadow-md.mb-8');
        if (kkSection) {
            kkSection.insertBefore(editBtn, kkSection.firstChild);
        }
    }
}
