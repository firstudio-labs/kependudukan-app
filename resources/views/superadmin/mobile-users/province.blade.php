<x-layout>
    <div class="p-4 mt-14">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Pengguna Mobile - {{ $province['name'] }}</h1>
                <p class="text-sm text-gray-500 mt-1">Daftar Kabupaten/Kota di Provinsi {{ $province['name'] }}</p>
            </div>
                ...
                <tbody>
                    @forelse($districts as $index => $district)
                        <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-center">{{ $district['mobile_users_count'] }}</td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('superadmin.mobile-users.district', [$province['id'], $district['id']]) }}" 
                                   class="text-blue-600 hover:text-blue-800 text-sm mr-3">Lihat Kecamatan</a>
                                <a href="{{ route('superadmin.mobile-users.detail', ['district', $district['id']]) }}" 
                                   class="text-blue-600 hover:text-blue-800 text-sm">Detail</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-6 text-center text-gray-500">Tidak ada data kabupaten/kota.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                <div class="text-sm text-gray-700">
                    ...
                    <span class="font-semibold text-[#7886C7]">{{ $districts->firstItem() ?? 0 }}</span>
                    <span class="font-medium text-gray-900">sampai</span> 
                    <span class="font-semibold text-[#7886C7]">{{ $districts->lastItem() ?? 0 }}</span>
                    <span class="font-medium text-gray-900">dari</span> 
                    <span class="font-semibold text-[#7886C7]">{{ $districts->total() }}</span>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadSubDistricts(districtId) {
                let districtData = window.districtData || [];
            ...
            function loadVillages(subDistrictId) {
                let subDistrictData = window.subDistrictData || [];
