<x-layout>
    <div class="p-4 mt-14">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Pengguna Mobile - {{ $district['name'] }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ $province['name'] }} → {{ $district['name'] }}</p>
            </div>
            <a href="{{ route('superadmin.mobile-users.province', $province['id']) }}" class="px-4 py-2 text-sm rounded-lg bg-gray-500 text-white hover:bg-gray-600">Kembali</a>
        </div>
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <select name="district_id" id="district_id" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" {{ !($provinceId ?? null) ? 'disabled' : '' }}>
                        ...
                        @if(isset($districts) && count($districts) > 0)
                            @foreach($districts as $dist)
                                <option value="{{ $dist['id'] }}" {{ ($districtId ?? null) == $dist['id'] ? 'selected' : '' }}>
                                    {{ $dist['name'] }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div>
                    ...
                    <select name="sub_district_id" id="sub_district_id" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" {{ !($districtId ?? null) ? 'disabled' : '' }}>
                        ...
                        @if(isset($subDistrictsList) && count($subDistrictsList) > 0)
                            @foreach($subDistrictsList as $subDist)
                                <option value="{{ $subDist['id'] }}" {{ ($subDistrictId ?? null) == $subDist['id'] ? 'selected' : '' }}>
                                    {{ $subDist['name'] }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <select name="village_id" id="village_id" class="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" {{ !($subDistrictId ?? null) ? 'disabled' : '' }}>
                        ...
                        @if(isset($villages) && count($villages) > 0)
                            @foreach($villages as $vill)
                                <option value="{{ $vill['id'] }}" {{ ($villageId ?? null) == $vill['id'] ? 'selected' : '' }}>
                                    {{ $vill['name'] }}
                                </option>
                            @endforeach
                        @endif
                ...
                <tbody>
                    @forelse($subDistricts as $index => $subDistrict)
                        <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                            <td class="px-6 py-4 font-medium">{{ $subDistrict['name'] }}</td>
                            <td class="px-6 py-4 text-center">{{ $subDistrict['mobile_users_count'] }}</td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('superadmin.mobile-users.subdistrict', [$province['id'], $district['id'], $subDistrict['id']]) }}" 
                                   class="text-blue-600 hover:text-blue-800 text-sm mr-3">Lihat Desa/Kelurahan</a>
                                <a href="{{ route('superadmin.mobile-users.detail', ['subdistrict', $subDistrict['id']]) }}" 
                                   class="text-blue-600 hover:text-blue-800 text-sm">Detail</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-6 text-center text-gray-500">Tidak ada data kecamatan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                <div class="text-sm text-gray-700">
                    ...
                    <span class="font-semibold text-[#7886C7]">{{ $subDistricts->firstItem() ?? 0 }}</span>
                    <span class="font-medium text-gray-900">sampai</span> 
                    <span class="font-semibold text-[#7886C7]">{{ $subDistricts->lastItem() ?? 0 }}</span>
                    <span class="font-medium text-gray-900">dari</span> 
                    <span class="font-semibold text-[#7886C7]">{{ $subDistricts->total() }}</span>
