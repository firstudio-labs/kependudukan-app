<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Buat Surat Keterangan Domisili</h1>

        <form method="POST" action="{{ route('superadmin.surat.domisili.store') }}" class="bg-white p-6 rounded-lg shadow-md">
            @csrf
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function initializeSelect2WithData() {
                ...
                $('#nikSelect').on('select2:select', function (e) {
                    ...
                    if (citizen) {
                        ...
                        // Fix job_type_id handling - ensure it's stored as a string
                        const jobTypeId = citizen.job_type_id ? citizen.job_type_id.toString() : '';
                        ...
                        // Set RT field with original RT value without converting to number
                        $('#rt').val(citizen.rt || '');

                        // Note: We're NOT auto-populating location fields now
                        // Domicile address might be different from registered address
                        // So we leave domicile_address empty for manual entry
                    }
                ...
                $('#fullNameSelect').on('select2:select', function (e) {
                    ...
                    if (citizen) {
                        ...
                        // Set RT field with original RT value without any conversion
                        $('#rt').val(citizen.rt || '');

                        // Note: We're NOT auto-populating location fields now
                        // Leave domicile_address empty for manual entry
                    }
            ...
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                // Validate NIK and full_name from Select2
                const nikValue = $('#nikSelect').val();
                const fullNameValue = $('#fullNameSelect').val();
                ...
                if (!fullNameValue) {
                    alert('Silakan pilih Nama Lengkap terlebih dahulu');
                    return false;
                }
