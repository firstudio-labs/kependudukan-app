<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Daftar Klasifikasi</h1>

        <div class="flex justify-between items-center mb-4">
            <form method="GET" action="" class="relative w-full max-w-xs">
                <input type="text" name="search" id="search" value="{{ request('search') }}"
                    class="block p-2 pl-10 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Cari data Klasifikasi..." />
            ...
            <div class="flex space-x-2">
                <button type="button"
                    onclick="window.location.href='{{ route('superadmin.datamaster.klasifikasi.create') }}'"
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-[#e6e8ed]">
                    <tr>
                        ...
                        <th class="px-6 py-3">Jenis Klasifikasi</th>
                ...
                <tbody>
                    ...
                    @forelse($klasifikasi as $index => $item)
                        <tr class="bg-white border-gray-300 border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">{{ $item->jenis_klasifikasi }}</td>
                            <td class="px-6 py-4">{{ $item->keterangan ?? '-' }}</td>
                            <td class="flex items-center px-6 py-4 space-x-2">
                                <a href="{{ route('superadmin.datamaster.klasifikasi.edit', $item->id) }}"
                                    class="text-yellow-600 hover:text-yellow-800" aria-label="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                                <form action="{{ route('superadmin.datamaster.klasifikasi.destroy', $item->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4">Tidak ada data klasifikasi</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="px-4 py-3 flex flex-col sm:flex-row justify-between items-center">
                <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                    Showing {{ $klasifikasi->firstItem() ?? 0 }} to {{ $klasifikasi->lastItem() ?? 0 }} of
                    {{ $klasifikasi->total() ?? 0 }} results
                </div>
                @if($klasifikasi->hasPages())
                    <nav class="relative z-0 inline-flex shadow-sm -space-x-px" aria-label="Pagination">
                        <!-- Previous Button -->
                        @if($klasifikasi->onFirstPage())
                            <span
                        ...
                        @else
                            <a href="{{ $klasifikasi->previousPageUrl() }}&search={{ request('search') }}"
                        ...
                        <!-- Page Numbers -->
                        @php
                            $currentPage = $klasifikasi->currentPage();
                            $lastPage = $klasifikasi->lastPage();
                        ...
                        <!-- First Page & Ellipsis -->
                        @if($startPage > 1)
                            <a href="{{ $klasifikasi->url(1) }}&search={{ request('search') }}"
                        ...
                        @for($i = $startPage; $i <= $endPage; $i++)
                            <a href="{{ $klasifikasi->url($i) }}&search={{ request('search') }}"
                        ...
                        @if($endPage < $lastPage)
                            ...
                            @endif
                            <a href="{{ $klasifikasi->url($lastPage) }}&search={{ request('search') }}"
                                class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">
                                {{ $lastPage }}
                            </a>
                        @endif
                        ...
                        <!-- Next Button -->
                        @if($klasifikasi->hasMorePages())
                            <a href="{{ $klasifikasi->nextPageUrl() }}&search={{ request('search') }}"
