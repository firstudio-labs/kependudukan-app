<x-layout>
    <div class="p-4 mt-14">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">Tambah Warungku</h1>
        <p class="text-sm text-gray-500 mb-6">Isi data klasifikasi dan jenis untuk master Warungku.</p>

        <form method="POST" action="{{ route('superadmin.datamaster.warungku.store') }}" class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    ...
                    <select name="klasifikasi" required
                        ...
                        <option value="" {{ old('klasifikasi') ? '' : 'selected' }}>Pilih</option>
                        <option value="barang" {{ old('klasifikasi')==='barang' ? 'selected' : '' }}>Barang</option>
                        <option value="jasa" {{ old('klasifikasi')==='jasa' ? 'selected' : '' }}>Jasa</option>
                    </select>
                    @error('klasifikasi')
