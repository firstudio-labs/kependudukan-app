...
function forceUpdateAge() {
    ...
    if (
        ...
        ageInput.value = "";
        console.log(
            "⚠️ Force update cleared age - invalid date:",
            currentValue
        );
        return null;
    }
...
function testAgeUpdate() {
    ...
    console.log("Birth date value:", birthDateInput.value);
    console.log("Current age value:", ageInput.value);

    // Test with a specific date
    birthDateInput.value = "1990-01-01";
    handleBirthDateChange({ target: birthDateInput, type: "test" });
...
window.testAgeUpdate = testAgeUpdate;
window.forceUpdateAge = forceUpdateAge;
window.handleBirthDateChange = handleBirthDateChange;
window.calculateAge = calculateAge;

// Debug function to manually trigger age update
window.debugAgeUpdate = function () {
    console.log("🔧 Debug: Manual age update triggered");
    const birthDateInput = document.getElementById("birth_date");
    const ageInput = document.getElementById("age");
    ...
    console.log(
        "  Birth date:",
        `"${birthDateInput ? birthDateInput.value : "NOT FOUND"}"`
    );
    console.log("  Age:", `"${ageInput ? ageInput.value : "NOT FOUND"}"`);

    if (birthDateInput && ageInput) {
        handleBirthDateChange({ target: birthDateInput, type: "debug" });
    }
};

// Debug function to check if event listeners are attached
window.debugEventListeners = function () {
    ...
    console.log("🎧 Checking event listeners on birth_date input:");
...
function handleBirthDateChange(event) {
    ...
    console.log("📝 Current age input value:", `"${ageInput.value}"`);
    ...
    if (birthDateValue && /^\d{4}-\d{2}-\d{2}$/.test(birthDateValue)) {
        try {
            ...
            ageInput.value = age;
            console.log("✅ Age input updated to:", age);

            // Force trigger change event on age input for any other listeners
        ...
        if (birthDateValue) {
            console.log("⚠️ Birth date format invalid:", `"${birthDateValue}"`);
        } else {
            console.log("📭 Birth date is empty, clearing age");
        }
        ageInput.value = "";
    }
}
...
function forceSyncFormWithData() {
    ...
    setSelectValueDirectly(
        "marital_certificate",
        citizenData.marital_certificate
    );
    setSelectValueDirectly(
        "divorce_certificate",
        citizenData.divorce_certificate
    );
...
function processRfIdValue(rfIdValue, inputElement) {
    ...
    if (
        typeof window.allCitizens !== "undefined" &&
        Array.isArray(window.allCitizens)
    ) {
        allCitizens = window.allCitizens;
    } else if (
        typeof window.citizens !== "undefined" &&
        Array.isArray(window.citizens)
    ) {
    ...
    const matchedCitizen = allCitizens.find((citizen) => {
        ...
        const normalizedStored = citizen.rf_id_tag
            .toString()
            .replace(/^0+/, "")
            .trim();
...
function populateCitizenDataForUpdate(citizen) {
    ...
    if (
        ...
        if (formattedDate && /^\d{4}-\d{2}-\d{2}$/.test(formattedDate)) {
            ...
            console.log(
                "⚠️ Invalid birth date format from RF ID:",
                formattedDate
            );
            $("#birth_date").val("");
            $("#age").val("");
    ...
    if (citizen.divorce_certificate_date) {
        ...
        if (citizen.divorce_certificate_date.includes("/")) {
            const [day, month, year] =
                citizen.divorce_certificate_date.split("/");
            const formattedDate = `${year}-${month.padStart(
                2,
                "0"
            )}-${day.padStart(2, "0")}`;
            ...
            $("#divorce_certificate_date").val(
                citizen.divorce_certificate_date
            );
        }
    }
